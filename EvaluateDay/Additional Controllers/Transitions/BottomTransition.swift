...
class BottomTransition: Transition {

    func presentAnimation(interactive: Bool) {
        self.toViewController.view.frame = self.containerView.frame
        self.containerView.addSubview(self.toViewController.view)
        
        guard let toViewController = self.toViewController as? BottomViewController else {
    ...
    func dismissAnimation(interactive: Bool) {
        ...
        guard let fromViewController = self.fromViewController as? BottomViewController else {
            self.transitionContext.completeTransition(false)
            return
        }
