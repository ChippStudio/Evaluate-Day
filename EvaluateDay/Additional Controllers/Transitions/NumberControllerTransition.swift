...
class NumberControllerTransition: Transition {

    func presentAnimation(interactive: Bool) {
        self.toViewController.view.frame = self.containerView.frame
        self.containerView.addSubview(self.toViewController.view)
        
        guard let toViewController = self.toViewController as? NumberViewController else {
            self.transitionContext.completeTransition(false)
            return
        }
        
        let maskAlpha = toViewController.maskView.alpha
        toViewController.contentView.transform = CGAffineTransform(translationX: 0.0, y: self.containerView.frame.size.height)
    ...
    func dismissAnimation(interactive: Bool) {
        ...
        guard let fromViewController = self.fromViewController as? NumberViewController else {
            self.transitionContext.completeTransition(false)
            return
        }
        
        UIView.animateKeyframes(withDuration: self.transitionDuration, delay: 0.0, options: .allowUserInteraction, animations: {
            fromViewController.contentView.transform = CGAffineTransform(translationX: 0.0, y: self.containerView.frame.size.height)
