...
class CriterionTenAnalyticsSection: ListSectionController, ASSectionController, AnalyticalSection {
    ...
    @objc private func shareAction(sender: ASButtonNode) {
        ...
        if self.lineChartData != nil {
            inView = AnalyticsLineChartShareView(image: self.lineChartData!.0, xValue: self.lineChartData!.1, yValue: self.lineChartData!.2, positive: (self.card.data as! CriterionTenCard).positive, title: self.card.title, subtitle: self.card.subtitle, type: self.card.type)
            self.lineChartData = nil
        } else if self.barChartData != nil {
            inView = AnalyticsBarChartShareView(image: self.barChartData!.0, xValue: self.barChartData!.1, yValue: self.barChartData!.2, positive: (self.card.data as! CriterionTenCard).positive, title: self.card.title, subtitle: self.card.subtitle, type: self.card.type)
