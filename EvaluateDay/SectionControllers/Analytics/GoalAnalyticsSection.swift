...
class GoalAnalyticsSection: ListSectionController, ASSectionController, AnalyticalSection {
        ...
        case .average:
            ...
            if let first = sortedValues.first {
                ...
                while currentMonthEnd.timeIntervalSince1970 > runDate.timeIntervalSince1970 {
                    ...
                    let monthValues = goalCard.values.filter("(created >= %@) AND (created <= %@)", monthInterval.start, monthInterval.end)
    ...
    override func didSelectItem(at index: Int) {
        if self.nodes[index] == .more {
            ...
            controller.values = (self.card.data as! GoalCard).values.sorted(byKeyPath: "created", ascending: false)
