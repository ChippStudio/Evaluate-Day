...
class TrackerAnalyticsSection: ListSectionController, ASSectionController, AnalyticalSection, FSCalendarDelegate, FSCalendarDelegateAppearance {
    ...
    func nodeBlockForItem(at index: Int) -> ASCellNodeBlock {
        ...
        case .calendar:
            return {
                let node = AnalyticsCalendarNode(title: Localizations.Analytics.Tracker.Calendar.title.uppercased(), isPro: true)
        ...
        case .monthTotal:
            ...
            if let first = sortedValues.first {
                ...
                while currentMonthEnd.timeIntervalSince1970 > runDate.timeIntervalSince1970 {
                    ...
                    let monthValues = trackerCard.values.filter("(created >= %@) AND (created <= %@)", monthInterval.start, monthInterval.end)
