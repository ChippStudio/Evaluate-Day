...
class PhraseMergeSection: ListSectionController, ASSectionController, MergeSection {
    // MARK: - Variable
    var card: Card!
            ...
            return {
                ...
                node.didPressed = { () in
                    if self.selectedIndex == nil {
                        ...
                        for value in (otherCard.data as! PhraseCard).values {
                            if let alreadyIn = (self.card.data as! PhraseCard).values.filter("(created >= %@) AND (created <= %@)", value.created.start, value.created.end).first {
                                if !self.byBase {
