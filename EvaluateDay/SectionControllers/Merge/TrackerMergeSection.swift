...
class TrackerMergeSection: ListSectionController, ASSectionController, MergeSection {
    // MARK: - Variable
    var card: Card!
            ...
            return {
                ...
                node.didPressed = { () in
                    if self.selectedIndex == nil {
                        ...
                        for value in (otherCard.data as! TrackerCard).values {
                            if let alreadyIn = (self.card.data as! TrackerCard).values.filter("(created >= %@) AND (created <= %@)", value.created.start, value.created.end).first {
                                if !self.byBase {
                                    if alreadyIn.edited < value.edited {
                                        try! Database.manager.data.write {
                                            alreadyIn.text = value.text
